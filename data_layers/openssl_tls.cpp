...
OpenSSL_TLS::~OpenSSL_TLS() { destroy(); }
...
OpenSSL_TLS_Server::~OpenSSL_TLS_Server() { cleanup(); }
...
int bio_create(BIO *b) { return 1; }

long on_ctrl(BIO *b, int cmd, long l, void *p) { return 1; }
...
inline void LoadServerCert(const char *certificate_filepath, const char *private_key_filepath,
                           X509 **m_Cert, EVP_PKEY **m_PrivateKey)
{
    FILE *cert_file = 0;
    FILE *key_file = 0;
...
shared_ptr<OpenSSL_TLS> OpenSSL_TLS_Server::handshake(DataLayer *lower_l,
                                                      const char *certificate_filepath,
...
void OpenSSL_TLS_Client::LoadTrustedCerts()
{
    ...
    hCertStore = CertOpenStore(CERT_STORE_PROV_SYSTEM, 0, NULL, CERT_SYSTEM_STORE_CURRENT_USER,
                               L"Root");
    if (!hCertStore) {
...
std::shared_ptr<OpenSSL_TLS> OpenSSL_TLS_Client::handshake(DataLayer *lower_l,
...
OpenSSL_TLS_Client::OpenSSL_TLS_Client() { InitOpenSSL(); }

OpenSSL_TLS_Client::~OpenSSL_TLS_Client() { cleanup(); }
#endif
