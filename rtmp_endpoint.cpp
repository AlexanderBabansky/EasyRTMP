...
RTMPEndpoint::RTMPEndpoint(DataLayer* lower_l) :
...
DATA_BYTES RTMPEndpoint::GetRTMPMessage(RTMPMessageType* message_type, uint32_t* message_stream_id, uint32_t* message_length, uint8_t* _o_chunk_stream_id, uint64_t* timestamp) {
...
bool RTMPEndpoint::HandleMessage(DATA_BYTES data, RTMPMessageType message_type, uint32_t message_stream_id, int message_length, int chunk_stream_id, uint64_t timestamp)
{
	switch (message_type)
	{
...
void RTMPEndpoint::SendRTMPMessage(Serializable* data, uint8_t chunk_stream_id, uint32_t message_stream_id, RTMPMessageType message_type, uint64_t timestamp) {
	...
	while (bytes_sent != bb.size()) {
		...
		else if (cs.message_stream_id == message_stream_id &&
			timestamp >= cs.timestamp) {
			cbh.d.fmt = 1;
...
void RTMPEndpoint::SendCommand(AMFValue data, uint8_t chunk_stream_id, uint32_t message_stream_id, RTMPMessageType message_type) {
