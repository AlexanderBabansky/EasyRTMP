...
void RTMPClientSession::SendDataFrameParameters(const ClientParameters *params)
{
    ...
    AMFValue amf_props(AMFType::ECMA_ARRAY);
    {
        ...
        if (params->has_audio) {
            ...
            case 2: amf_props.at("stereo").set_boolean(true); break;
            }
        }
...
RTMPClientSession::RTMPClientSession(RTMPEndpoint *endpoint) : m_Endpoint(endpoint)
{
    assert(endpoint);
}
...
void RTMPClientSession::SendRTMPMessage(RTMPMediaMessage message)
{
    ...
    if (message.message_type == RTMPMessageType::AUDIO) {
        m_Endpoint->SendRTMPMessage(&message.audio, 4, message.message_stream_id,
                                    message.message_type, message.timestamp);
    } else if (message.message_type == RTMPMessageType::VIDEO) {
        m_Endpoint->SendRTMPMessage(&message.video, 4, message.message_stream_id,
                                    message.message_type, message.timestamp);
    } else
        assert(false);
}
