...
AMFValue::AMFValue(AMFValue&& o) noexcept {
	...
	{
	case AMFType::STRING:
		data_string = std::move(o.data_string);
		break;
	case AMFType::NUMBER:
		data_number = o.data_number;
		break;
	case AMFType::BOOLEAN:
		data_boolean = o.data_boolean;
		break;
	case AMFType::ECMA_ARRAY:
	case AMFType::OBJECT:
		data_object = std::move(o.data_object);
		break;
...
int AMFValue::GetLength() const {
	...
	{
	case AMFType::STRING:
		return data_string.size() + sizeof(uint16_t) + 1;
	case AMFType::NUMBER:
	...
	case AMFType::OBJECT:
	{
		...
		if (type == AMFType::ECMA_ARRAY)res += sizeof(uint32_t);
	...
	case AMFType::STRICT_ARRAY:
		throw rtmp_exception(RTMPError::AMF);
		break;
	case AMFType::ROOT:
	{
...
void AMFValue::ParseRef(const char*& data, int& data_len) {
	...
	case AMFType::BOOLEAN:
	{
		...
		if (b_d)data_boolean = true;
		else data_boolean = false;
	}
	break;
	}
}
...
AMFValue::iterator AMFValue::begin(){
	...
	case AMFType::ECMA_ARRAY:
	case AMFType::OBJECT:
		return AMFValue::iterator(type,data_object.begin());
	case AMFType::ROOT:
		return AMFValue::iterator(type, data_array.begin());
	default:
		throw rtmp_exception(RTMPError::AMF);
	}	
}
...
AMFValue::iterator AMFValue::end() {
	...
	case AMFType::ECMA_ARRAY:
	case AMFType::OBJECT:
		return AMFValue::iterator(type, data_object.end());
	case AMFType::ROOT:
		return AMFValue::iterator(type, data_array.end());
	default:
		throw rtmp_exception(RTMPError::AMF);
	}	
}
...
AMFValue::iterator::iterator(const iterator& o)
{
	...
	case AMFType::ECMA_ARRAY:
	case AMFType::OBJECT:
		map_iterator = o.map_iterator;
		break;
	case AMFType::ROOT:
		list_iterator = o.list_iterator;
		break;
	default:
		throw rtmp_exception(RTMPError::AMF);
	}	
...
bool AMFValue::iterator::operator==(const iterator& o) const
{
	...
	case AMFType::ROOT:
		return list_iterator == o.list_iterator;;
	default:
		throw rtmp_exception(RTMPError::AMF);
	}	
}
