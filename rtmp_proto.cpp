...
void C0::Serialize(char*& data) const
{
	WriteDataType<uint8_t>(data, d.version);
}
...
int ChunkBasicHeader::GetLength() const
{
...
int ChunkMessageHeader0::GetLength() const
{
	return 3 + 3 + 1 + 4;
}
...
int ChunkMessageHeader1::GetLength() const
{
	return 3 + 3 + 1;
}
...
int ChunkMessageHeader2::GetLength() const
{
	return 3;
}

void SetChunkSize::Serialize(char*& data) const
{
	WriteDataType<uint32_t>(data, chunk_size);
}
...
int SetChunkSize::GetLength() const
{
...
int AbortMessage::GetLength() const
{
	return sizeof(chunk_stream_id);
}
...
int Acknowledgement::GetLength() const
{
...
int WindowsAcknowledgementSize::GetLength() const
{
	return sizeof(acknowledgement_window_size);
}
...
int SetPeerBandwith::GetLength() const
{
	return sizeof(d);
}
...
int UCM_StreamBegin::GetLength() const
{
	return sizeof(type)+sizeof(stream_id);
}
...
int AudioPacketAAC::GetLength() const
{
	return sizeof(uint8_t) * 2 + audio_data_send.size();
}
...
int VideoPacket::GetLength() const
{
	return 2 * sizeof(uint8_t) + 3 + video_data_send.size();
}
